	...
	<body>
		...
		<div id="show-setor" class="content scaffold-show" role="main">
			...
			<ol class="property-list setor">
				...
				<g:if test="${setorInstance?.gruposCaseiros}">
				<li class="fieldcontain">
					<span id="gruposCaseiros-label" class="property-label"><g:message code="setor.gruposCaseiros.label" default="Grupos Caseiros" /></span>
					
						<g:each in="${setorInstance.gruposCaseiros}" var="g">
						<span class="property-value" aria-labelledby="gruposCaseiros-label"><g:link controller="grupoCaseiro" action="show" id="${g.id}">${g?.encodeAsHTML()}</g:link></span>
						</g:each>
